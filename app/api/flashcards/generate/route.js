...
export async function POST(request) {
  ...
  try {
    ...
    }    const { materialId, numFlashcards, title, description, generateImages, translateToArabic } = requestData;

    console.log(`📝 Generation request: ${numFlashcards} flashcards, images: ${generateImages ? 'enabled' : 'disabled'}, translation: ${translateToArabic ? 'Arabic' : 'disabled'}`);
    ...
    console.log(`📖 Content length: ${material.rawContent.length} characters`);    // Generate flashcards using FlashcardGenerator
    ...
    if (translationInfo?.enabled) {
      console.log(`🌐 Translation to Arabic: ${translationInfo.stats?.successRate || 0}% success rate`);
    }
    ...
    if (generateImages && process.env.FANAR_API_KEY) {
      ...
      try {        const imageGenerator = new ImageGenerator(process.env.FANAR_API_KEY, {
        ...
        const imageResults = await imageGenerator.generateFlashcardImages(flashcardsForImageGen, tempDeckId, false);
        ...
        flashcardsWithImages = flashcards.map((flashcard, index) => ({
          ...
          imageUrl: imageResults[index]?.imageResult?.success ? imageResults[index].imageResult.imageUrl : null
        }));
    ...
    console.log(`🎉 Total generation completed in ${Math.round(totalDuration / 1000)}s (${Math.round(totalDuration / 60000)}m)`);    return NextResponse.json({
      success: true,
      deck,
...
async function generateFlashcards(content, params, options = {}) {
  ...
  console.log(`📊 Content: ${content.length} characters, Target: ${numFlashcards} flashcards`);
  ...
  try {
    ...
    const result = await generator.generateFlashcardsWithTranslation(content, numFlashcards, {
      translateToArabic,
      translationApiKey: process.env.FANAR_API_KEY // Use same API key for translation
    });
    
    console.log(`✅ Smart generation complete: ${result.flashcards.length}/${numFlashcards} flashcards`);
    console.log(`📈 Success rate: ${Math.round((result.flashcards.length / numFlashcards) * 100)}%`);
