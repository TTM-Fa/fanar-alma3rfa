...
const Header = () => (
  <header className="mb-8 text-center">
    <h1 className="text-3xl font-bold text-[#223366]">
      Upload Study Materials
    </h1>
    <p className="mt-2 text-[#223366]">
...
const SessionHeader = ({ sessionDetails, isLoading }) => {
  if (isLoading) {
    return (
      <Card className="bg-[#223366] shadow-lg border rounded-lg overflow-hidden transition mb-8">
        <CardContent className="p-6">
          <div className="flex flex-col gap-3 animate-pulse">
            <div className="h-6 bg-[#b3c6e0] rounded w-3/4"></div>
            <div className="h-4 bg-[#b3c6e0] rounded w-1/4"></div>
            <div className="h-4 bg-[#b3c6e0] rounded w-full"></div>
            <div className="flex justify-between items-center mt-2">
              <div className="h-3 bg-[#b3c6e0] rounded w-1/3"></div>
              <div className="h-3 bg-[#b3c6e0] rounded w-1/4"></div>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <CardContent className="p-6">
        <div className="flex flex-col md:flex-row justify-between">
          <div className="mb-4 md:mb-0">
            ...
            <Badge className="mt-2 bg-white/20 text-white border-white">
              {sessionDetails.subject}
            </Badge>
            {sessionDetails.description && (
              <p className="text-white mt-2 line-clamp-2 md:line-clamp-none">
                {sessionDetails.description}
              </p>
            )}
          </div>
          <div className="flex flex-col items-end justify-center text-sm">
            <div className="flex items-center text-white mb-1">
              ...
              <span>
                Created: {new Date(sessionDetails.createdAt).toLocaleDateString()}
              </span>
            </div>
            <div className="flex items-center text-white">
              <File className="h-4 w-4 mr-1 text-white" />
              <span>
                {sessionDetails.materialsCount || sessionDetails.materials?.length || 0} Materials
              </span>
            </div>
          </div>
        </div>
...
const FileUpload = ({ handleDrop, handleFileChange, fileInputId }) => (
  <Card className="bg-[#223366] shadow-lg border border-[#223366] rounded-lg overflow-hidden">
    <CardContent className="p-6">
      <div className="bg-white rounded-lg p-6">
        <h2 className="text-xl font-bold text-[#223366] mb-4">Upload Files</h2>
        <div
          className="border border-dashed border-[#b3c6e0] rounded-lg p-6 text-center transition hover:border-[#223366] hover:bg-[#b3c6e0]/30"
          onDragOver={(e) => e.preventDefault()}
          onDrop={handleDrop}
        >
          <div className="flex flex-col items-center">
            <div className="flex items-center space-x-2 mb-2">
              <FileText className="h-8 w-8 text-[#223366]" />
              <FileAudio className="h-8 w-8 text-[#b3c6e0]" />
            </div>
            <p className="text-[#223366]">Drop your files here</p>
            <p className="text-sm text-[#223366]/70 mb-2">Supported formats:</p>
            <div className="flex flex-wrap justify-center gap-2 mb-4">
              <Badge variant="outline" className="bg-[#eaf0fa] text-[#223366] border-[#b3c6e0]">PDF</Badge>
              <Badge variant="outline" className="bg-[#eaf0fa] text-[#223366] border-[#b3c6e0]">MP3</Badge>
              <Badge variant="outline" className="bg-[#eaf0fa] text-[#223366] border-[#b3c6e0]">WAV</Badge>
              <Badge variant="outline" className="bg-[#eaf0fa] text-[#223366] border-[#b3c6e0]">M4A</Badge>
            </div>
            <Button
              onClick={() => document.getElementById(fileInputId).click()}
              className="mt-2 bg-gradient-to-r from-[#b3c6e0] to-[#223366] text-[#223366] font-bold border border-[#b3c6e0]"
            >
              Browse Files
            </Button>
            <input
        ...
        <p className="mt-4 text-xs text-[#223366]/70">Max file size: 30MB</p>
      </div>
    </CardContent>
  </Card>
);
...
const LinkUpload = ({
  ...
  <Card className="bg-[#223366] shadow-lg border rounded-lg overflow-hidden transition hover:shadow-xl opacity-50 cursor-not-allowed">
    <CardContent className="p-6">
      <div className="bg-white rounded-lg p-6">
        <h2 className="text-xl font-bold text-[#223366] mb-4">Add Links</h2>
        <div className="flex space-x-2 mb-4">
          <Input
            ...
            disabled={true}
            className="flex-1 border bg-[#eaf0fa] text-[#223366] placeholder-[#b3c6e0]"
          />
          <Button
            onClick={handleAddLink}
            disabled={true}
            className="bg-gradient-to-r from-[#b3c6e0] to-[#223366] text-[#223366] font-bold border border-[#b3c6e0]"
          >
            Add
          </Button>
        </div>
...
const PreviewSection = ({
  ...
  <Card className="bg-[#223366] shadow-lg border border-[#223366] rounded-lg overflow-hidden transition mt-6">
    <CardContent className="p-6">
      <div className="bg-white rounded-lg p-6">
        <h2 className="text-xl font-bold text-[#223366] text-center mb-6">
          Your Study Materials
        </h2>
        {files.length > 0 && (
          <div className="mb-8">
            <h3 className="text-lg text-[#223366] mb-4 border-b pb-2">
              Files
            </h3>
...
const IngestButton = ({ files, links, isUploading, handleIngest }) => (
  <div className="flex justify-center mt-6">
    <Button
      ...
      className="px-8 py-4 text-lg font-bold bg-gradient-to-r from-[#b3c6e0] to-[#223366] text-[#223366] rounded hover:bg-gradient-to-r hover:from-[#223366] hover:to-[#b3c6e0] transition disabled:opacity-70 border border-[#b3c6e0]"
    >
      {isUploading ? (
        <div className="flex items-center">
          <Loader className="mr-3 h-5 w-5 animate-spin text-[#223366]" />
          Processing...
        </div>
      ) : (
        "Ingest Materials"
...
const UploadMaterialsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#eaf0fa] flex flex-col p-6 md:p-10 relative overflow-hidden font-sans text-[#223366]">
      <div className="absolute top-0 left-0 w-1/3 h-1/3 bg-[#b3c6e0] rounded-full filter blur-3xl opacity-30 -z-10 -translate-x-1/2 -translate-y-1/2"></div>
      <div className="absolute bottom-0 right-0 w-1/2 h-1/2 bg-[#b3c6e0] rounded-full filter blur-3xl opacity-30 -z-10 translate-x-1/4 translate-y-1/4"></div>
      <div className="max-w-4xl w-full mx-auto">
        <div className="bg-[#eaf0fa] border border-[#b3c6e0] rounded-2xl p-6 md:p-10">
          <Header />
          <SessionHeader sessionDetails={sessionDetails} isLoading={isLoadingMaterials} />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FileUpload handleDrop={handleDrop} handleFileChange={handleFileChange} fileInputId="fileInput" />
            <LinkUpload currentLink={currentLink} setCurrentLink={setCurrentLink} handleAddLink={handleAddLink} links={links} handleRemoveLink={handleRemoveLink} youtubeLoading={youtubeLoading} disabled={true} />
          </div>
          {(files.length > 0 || links.length > 0) && (
            <PreviewSection files={files} links={links} isUploading={isUploading} processingStatus={processingStatus} processingPhases={processingPhases} formatFileSize={formatFileSize} getProgressGradient={getProgressGradient} handleRemoveFile={handleRemoveFile} handleRemoveLink={handleRemoveLink} getFileIcon={getFileIcon} navyMode={true} />
          )}
          {(files.length > 0 || links.length > 0) && (
            <div className="flex justify-center mt-6">
              <Button size="lg" onClick={handleIngest} disabled={isUploading || (files.length === 0 && links.length === 0)} className="px-8 py-4 text-lg font-bold bg-gradient-to-r from-[#b3c6e0] to-[#223366] text-[#223366] rounded hover:bg-gradient-to-r hover:from-[#223366] hover:to-[#b3c6e0] transition disabled:opacity-70 border border-[#b3c6e0]">
                {isUploading ? (
                  <div className="flex items-center">
                    <Loader className="mr-3 h-5 w-5 animate-spin text-[#223366]" />
                    Processing...
          ...
          <ExistingMaterialsSection existingMaterials={existingMaterials} isLoadingMaterials={isLoadingMaterials} loadError={loadError} processingStatus={processingStatus} getFileIcon={getFileIcon} formatFileSize={formatFileSize} onDeleteMaterial={handleDeleteMaterial} navyMode={true} />
          {isUploading && Object.values(processingStatus).every((status) => status.phase === processingPhases[status.type].length - 1 && status.progress === 100) && (
            <div className="mt-8 text-center">
              <CheckCircle className="mx-auto h-10 w-10 text-green-400" />
              <h3 className="text-xl font-bold text-[#223366] mt-2">Processing Complete</h3>
              <p className="text-[#223366]">Your materials are ready for review.</p>
            </div>
          )}
        </div>
      </div>
      <NotificationPopup
        ...
        action={notification.type === "processing" ? (
          <Button onClick={closeNotification} className="bg-[#223366] text-[#b3c6e0] hover:bg-[#1a2747] font-bold">Got it</Button>
        ) : null}
      />
    </div>
  );
