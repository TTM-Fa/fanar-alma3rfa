...
class FlashcardGenerator {
    ...
    calculateSmartChunking(text, targetFlashcards) {
        ...
        console.log(`   Safe char limit per chunk: ${safeCharLimit.toLocaleString()}`);
        ...
        const chunksNeededForDiversity = Math.max(this.minChunksForDiversity, Math.ceil(targetFlashcards / this.maxFlashcardsPerChunk));
        ...
        const baseFlashcardsPerChunk = Math.floor(targetFlashcards / finalChunkCount);
        const extraFlashcards = targetFlashcards % finalChunkCount;
        
        console.log(`   Final chunk count: ${finalChunkCount}`);
        console.log(`   Chunk size: ~${baseChunkSize.toLocaleString()} chars (+ ${overlapSize} overlap)`);
        console.log(`   Flashcards per chunk: ${baseFlashcardsPerChunk} (+ ${extraFlashcards} distributed)`);
        
        return {
            chunkCount: finalChunkCount,
            chunkSize: baseChunkSize,
            overlapSize: overlapSize,
            baseFlashcardsPerChunk: Math.max(this.minFlashcardsPerChunk, baseFlashcardsPerChunk),
    ...
    createSmartChunks(text, strategy) {
        ...
        for (let i = 0; i < chunkCount; i++) {
            ...
            const maxOffset = Math.min(chunkSize * 0.1, totalLength - baseStart - chunkSize);
        ...
        chunks.forEach(chunk => {
            console.log(`   Chunk ${chunk.id}: ${chunk.text.length} chars, ~${chunk.tokens} tokens`);
        });
     ...
     */
    async generateFlashcardsForChunk(chunkText, flashcardCount, chunkIndex, additionalInstruction = '') {        
...
IMPORTANT: Return exactly ${flashcardCount} flashcard objects in the array.`;
        ...
        try {
            console.log(`🔄 Generating ${flashcardCount} flashcards for chunk ${chunkIndex + 1}...`);
            ...
            console.log(`✅ Raw response for chunk ${chunkIndex + 1}:`, content.substring(0, 200) + '...');
            ...
            if (parsed && parsed.flashcards && Array.isArray(parsed.flashcards)) {
                ...
                if (actualCount > flashcardCount) {
                    console.warn(`⚠️ API returned ${actualCount} flashcards, expected ${flashcardCount}. Limited to ${flashcardCount}.`);
                }
                console.log(`✅ Successfully generated ${Math.min(actualCount, flashcardCount)} flashcards for chunk ${chunkIndex + 1}`);
            ...
            console.error(`❌ Error generating flashcards for chunk ${chunkIndex + 1}:`, error.message);
    ...
    parseFlashcardResponse(content, expectedCount = null) {
        try {
            ...
            if (parsed.flashcards && Array.isArray(parsed.flashcards)) {
                ...
                const flashcards = expectedCount ? parsed.flashcards.slice(0, expectedCount) : parsed.flashcards;
                return { flashcards };
            }
            return parsed;
        } catch (e) {
        ...
        try {
            ...
            if (questions.length === answers.length && questions.length > 0) {
                ...
                // Final safety limit
                const limitedFlashcards = expectedCount ? flashcards.slice(0, expectedCount) : flashcards;
                console.log(`🔧 Manual extraction: found ${flashcards.length}, limited to ${limitedFlashcards.length}`);
    ...
    async generateFlashcards(text, targetCount) {
        ...
        const allFlashcards = await this.generateInitialFlashcards(chunks, strategy);
        
        // Remove duplicates
        const uniqueFlashcards = this.removeDuplicateFlashcards(allFlashcards);
        console.log(`🔍 After deduplication: ${uniqueFlashcards.length} unique flashcards`);
        ...
        console.log(`🎯 Generation complete: ${finalFlashcards.length}/${targetCount} flashcards created`);
        console.log(`📊 Success rate: ${Math.round((finalFlashcards.length / targetCount) * 100)}%`);
    ...
    async generateInitialFlashcards(chunks, strategy) {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            flashcardsForThisChunk = Math.min(flashcardsForThisChunk, this.maxFlashcardsPerChunk);
            ...
            try {
                ...
                if (chunkFlashcards && chunkFlashcards.length > 0) {
                    ...
                    console.log(`✅ Added ${chunkFlashcards.length} flashcards from chunk ${i + 1}`);
                } else {
     ...
     */
    async ensureTargetCount(currentFlashcards, targetCount, chunks, originalText) {
        const currentCount = currentFlashcards.length;
        
        if (currentCount >= targetCount) {
            console.log(`✅ Target reached: ${currentCount}/${targetCount} flashcards`);
            return currentFlashcards.slice(0, targetCount);
        }
        
        const shortfall = targetCount - currentCount;
        console.log(`📋 Phase 2: Need ${shortfall} more flashcards to reach target of ${targetCount}`);
        ...
        if (shortfall > 0 && chunks.length > 0) {
            console.log(`🔄 Strategy 1: Generating additional flashcards from existing chunks`);
        ...
        if (additionalFlashcards.length < shortfall && originalText.length > 1000) {
            ...
            console.log(`🔄 Strategy 2: Creating targeted chunks for ${remainingShortfall} more flashcards`);
        ...
        if (finalUnique.length < targetCount) {
            console.log(`⚠️ Final count ${finalUnique.length} is still short of ${targetCount}`);
    ...
    async generateAdditionalFromChunks(chunks, needed, existingFlashcards) {
        ...
        for (let i = 0; i < sortedChunks.length && additionalFlashcards.length < needed; i++) {
            const chunk = sortedChunks[i];
            const flashcardsToRequest = Math.min(3, needed - additionalFlashcards.length);
            
            try {
                console.log(`🔄 Requesting ${flashcardsToRequest} additional flashcards from chunk ${chunk.originalIndex + 1}`);
                ...
                const newFlashcards = chunkFlashcards.filter(card => 
                ...
                if (newFlashcards.length > 0) {
                    ...
                    console.log(`✅ Added ${newFlashcards.length} new flashcards from chunk ${chunk.originalIndex + 1}`);
                }
                ...
                console.error(`❌ Failed to generate additional flashcards from chunk ${chunk.originalIndex + 1}:`, error.message);
                continue;
            }
        }
    ...
    async generateFromTargetedChunks(text, needed, existingFlashcards) {
        ...
        console.log(`🎯 Creating ${Math.min(3, Math.ceil(needed / 2))} targeted chunks for ${needed} flashcards`);
        
        // Create smaller, focused chunks from different parts of the text
        const targetedChunks = this.createTargetedChunks(text, Math.min(3, Math.ceil(needed / 2)));
        const additionalFlashcards = [];
        
        for (let i = 0; i < targetedChunks.length && additionalFlashcards.length < needed; i++) {
            ...
            try {
                console.log(`🎯 Generating ${flashcardsToRequest} targeted flashcards from chunk ${i + 1}`);
                ...
                console.error(`❌ Failed to generate targeted flashcards from chunk ${i + 1}:`, error.message);
                continue;
            }
        }
    ...
    createTargetedChunks(text, numChunks) {
        ...
        for (let i = 0; i < numChunks; i++) {
            ...
            const chunkStart = sectionStart + Math.floor(Math.max(0, sectionLength - safeChunkSize) / 2);
    ...
    removeDuplicateFlashcards(flashcards) {
        ...
        for (const card of flashcards) {
            ...
            const normalizedQuestion = card.question.toLowerCase().trim().replace(/[^\w\s]/g, '');
        ...
        console.log(`🔍 Removed ${flashcards.length - unique.length} duplicate flashcards`);
        return unique;
    }
    ...
    async generateFlashcardsWithTranslation(text, targetCount, options = {}) {
        ...
        console.log(`🚀 Starting flashcard generation${translateToArabic ? ' with Arabic translation' : ''}...`);
        ...
        if (translateToArabic && translationApiKey) {
            console.log(`🌐 Starting Arabic translation for ${flashcards.length} flashcards...`);
            ...
            try {
                ...
                const translatedFlashcards = await translator.translateFlashcards(flashcards, 'ar');
                
                const stats = translator.getTranslationStats(translatedFlashcards);
                console.log(`🌐 Translation complete: ${stats.successRate}% success rate`);
